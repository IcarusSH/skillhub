...
# DingTalk scan-code direct-login toggles. Keep the env-var defaults aligned
# with their server-side counterparts so leaving them unset yields a
# consistent "disabled" state across both halves of the request flow.
: "${SKILLHUB_WEB_AUTH_DINGTALK_ENABLED:=false}"
: "${SKILLHUB_WEB_AUTH_DINGTALK_APP_ID:=}"
: "${SKILLHUB_WEB_AUTH_DINGTALK_PROVIDER:=dingtalk}"
: "${SKILLHUB_WEB_AUTH_DINGTALK_REDIRECT_PATH:=/auth/dingtalk/callback}"

# Generate runtime-config.js
envsubst '${SKILLHUB_WEB_API_BASE_URL} ${SKILLHUB_PUBLIC_BASE_URL} ${SKILLHUB_WEB_AUTH_DIRECT_ENABLED} ${SKILLHUB_WEB_AUTH_DIRECT_PROVIDER} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_ENABLED} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_PROVIDER} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_AUTO} ${SKILLHUB_WEB_AUTH_DINGTALK_ENABLED} ${SKILLHUB_WEB_AUTH_DINGTALK_APP_ID} ${SKILLHUB_WEB_AUTH_DINGTALK_PROVIDER} ${SKILLHUB_WEB_AUTH_DINGTALK_REDIRECT_PATH}' \
